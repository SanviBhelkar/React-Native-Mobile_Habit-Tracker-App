...
const index = () => {
  ...
  const fetchHabits = async () => {
    try {
      const response = await axios.get("http://192.168.11.69:3000/habitsList");
  ...
  const handleCompletion = async () => {
    try {
      ...
      await axios.put(`http://192.168.11.69:3000/habits/${habitId}/completed`, {
  ...
  const deleteHabit = async () => {
    try {
      ...
      const response = await axios.delete(
        `http://192.168.11.69:3000/habits/${habitId}`
      );
  ...
  return (
    <>
      <ScrollView style={{ flex: 1, backgroundColor: "white", padding: 10 }}>
        ...
        <Text style={{ marginTop: 5, fontWeight: "500", fontSize: 23 }}>
          Habits
        </Text>
        <View
          style={{
